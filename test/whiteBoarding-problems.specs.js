...
describe('myIndexOf', function () {
    it('should not call `Array.indexOf` or `Array.includes`', function () {
        // expect(fizzBuzz(20)).to.deep.equal([3, 5, 6, 9, 10, 12, 18]);
        let arr = [1, 2, 3, 4];
        let indexOfSpy = chai.spy.on(arr, 'indexOf');
        let includesSpy = chai.spy.on(arr, 'includes');
        myIndexOf(arr, 4);
...
describe('abbreviate', function () {
    it('should take in a sentence string and return a new sentence where words longer than 4 charsacters have their vowels removed', function () {
...
describe('countScores', function () {
    it('should take in an array of score objects, and return an object where each name is `key` and  `value` is their total `score`', function () {
        const ppl = [
